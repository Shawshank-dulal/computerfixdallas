...
const Blogs = () => {
  ...
  const fetchData = async (currentPage) => {
    ...
    // Update URL to reflect the current page and pageSize in the query string
    router.push(`/blogs?page=${currentPage}&pageSize=${pageSize}`, undefined, { shallow: true });
  };
    ...
    <div>
      ...
      <div className="flex justify-center mt-10">
        <button
          ...
          disabled={page === 1}
          className={`px-4 py-2 mx-2 ${page === 1 ? "cursor-not-allowed" : "cursor-pointer"}`}
        >
          Previous
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 mx-2 ${page === pagination.pageCount ? "cursor-not-allowed" : "cursor-pointer"}`}
        >
          Next
        </button>
      </div>
